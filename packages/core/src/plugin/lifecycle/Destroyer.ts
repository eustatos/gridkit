...
export async function destroyPlugins(
  ...
  const { parallel = true, timeout = 10000, failFast = true } = options;
        ...
        if (timeout > 0) {
          destroyPromise = Promise.race([
            ...
            new Promise<void>((_, reject) => {
              setTimeout(
                () =>
                  reject(new Error(`Plugin ${pluginId} destruction timeout`)),
                timeout
              );
            }),
          ]);
    ...
    if (failFast) {
      ...
      for (let i = 0; i < destroyResults.length; i++) {
        ...
        if (result.status === 'fulfilled') {
          ...
          // For failFast, we throw on the first error
          const plugin = plugins[i];
          if (!plugin) {
            throw new Error('Plugin not found at index ' + i);
          }
          const pluginId = plugin.metadata.id;
          const error = result.reason as Error;
          throw new PluginDestructionError(pluginId, error);
        }
      }
    } else {
