...
describe('QuotaManager', () => {
  ...
  describe('setQuota', () => {
    it('should set quotas for a plugin', () => {
      ...
      expect(quotaManager.checkQuota('test-plugin', 'maxEventsPerSecond', 50)).toBe(true);
    });
  });
  ...
  describe('checkQuota', () => {
    ...
    it('should return false if plugin exceeds quota', () => {
      ...
      quotaManager.setQuota('test-plugin', { maxEventsPerSecond: 100 });
      expect(quotaManager.checkQuota('test-plugin', 'maxEventsPerSecond', 150)).toBe(false);
    ...
    it('should return true if no quota is set for the plugin', () => {
      expect(quotaManager.checkQuota('test-plugin', 'maxEventsPerSecond', 1000)).toBe(true);
    });
  ...
  describe('onQuotaExceeded', () => {
    it('should log a warning when quota is exceeded', () => {
      ...
      (quotaManager as any).onQuotaExceeded('test-plugin', 'maxEventsPerSecond', 100);
      
      expect(warnSpy).toHaveBeenCalledWith('Plugin test-plugin exceeded quota for maxEventsPerSecond: 100');
