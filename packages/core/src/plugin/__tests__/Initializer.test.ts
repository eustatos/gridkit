...
describe('Initializer', () => {
  describe('initializePlugin', () => {
    ...
    it('should throw PluginInitializationError on initialization failure', async () => {
      ...
      await expect(initializePlugin(plugin, context)).rejects.toThrow(PluginInitializationError);
      await expect(initializePlugin(plugin, context)).rejects.toThrow('Failed to initialize plugin failing-plugin: Initialization failed');
    });
  });
  ...
  describe('initializePlugins', () => {
    it('should initialize multiple plugins successfully', async () => {
      ...
      expect(initializeMock1).toHaveBeenCalledWith({ id: 'plugin-1' }, contexts.get('plugin-1'));
    ...
    it('should collect all results with failFast=false', async () => {
      ...
      const results = await initializePlugins([plugin1, plugin2], contexts, { failFast: false });
