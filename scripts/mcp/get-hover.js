#!/usr/bin/env node
/**
 * Get hover/type information using ts-language-mcp
 * 
 * Usage:
...
if (!filePath || !lineStr || !columnStr) {
  console.error('Usage: node scripts/mcp/get-hover.js <file> <line> <column>');
  console.error('Example: node scripts/mcp/get-hover.js packages/core/src/types/base.ts 45 20');
  process.exit(1);
}
...
child.on('close', (code) => {
  try {
    ...
    if (response.result?.content?.[0]) {
      ...
      console.log('No hover information found');
    }
  } catch (e) {
